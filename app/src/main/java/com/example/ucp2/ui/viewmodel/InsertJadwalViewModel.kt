...
class InsertJadwalViewModel(
    ...
    private fun validateFields(): Boolean {
        ...
        val errorState = FormErrorStateJadwal(
            idJadwal = if (event.idJadwal.isNotEmpty()) null
            else "ID tidak boleh kosong",
            namaDokter = if (event.namaDokter.isNotEmpty()) null
            else "Tidak boleh kosong",
            namaPasien = if (event.namaPasien.isNotEmpty()) null
            else "Tidak boleh kosong",
            noHp = if (event.noHp.isNotEmpty()) null
            else "Tidak boleh kosong",
            tanggalKonsultasi = if (event.tanggalKonsultasi.isNotEmpty()) null
            else "Tidak boleh kosong",
            status = if (event.status.isNotEmpty()) null
            else "Tidak boleh kosong"
        )
